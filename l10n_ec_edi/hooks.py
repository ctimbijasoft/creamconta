...
def post_init_hook(cr, registry):
    ...
    with open(csv_path, 'r') as csv_file:
        for row in csv.DictReader(csv_file, delimiter='|', fieldnames=['l10n_ec_edi_code', 'name', 'state_xml_id']):
            state = env.ref('base.%s' % row['state_xml_id'], raise_if_not_found=False)
            res_city_vals_list.append({
                'l10n_ec_edi_code': row['l10n_ec_edi_code'],
                'name': row['name'],
    ...
    if cities:
        cr.execute('''
           INSERT INTO ir_model_data (name, res_id, module, model, noupdate)
               SELECT 
                    'res_city_mx_' || lower(res_country_state.code) || '_' || res_city.l10n_ec_edi_code,
                    res_city.id,
                    'l10n_ec_edi',
                    'res.city',
                    TRUE
