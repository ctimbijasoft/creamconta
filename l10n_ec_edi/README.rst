...
- Add the personalized number related to the customs information, separated by commas, 
  ...
  * Customs: Code of customs clearance agent. Positions five and six must correspond to a key from the catalog of Customs (catSRI: c_Aduana)

  * Patent: Positions nine through twelve must correspond to a patent number of the catalog of customs patents (catSRI: c_PatenteAduanal)
  ...
  * Quantity: The value of the last six digits must be between the minimum value 1 and the value maximum of consecutive numbers in the catalog quantity column catSRI: c_NumPedimentoAduana that correspond to those used by customs in that year.
...
This module adds the External Trade Complement to SRI version 2_20, in which
...
- In the invoice:
  ...
  - **Need external trade?**: This field is used to indicate that in the SRI
...
- In the product:
  ...
  - **Weight**: If the *UMT Customs* is `KG`, here must be specific the weight
    ...
    .. figure:: ../l10n_ec_edi/static/src/Product_CET.png
...
- In Unit of measurement
  ...
  - **Customs Code**: Code that corresponding to the unit of measurement in the
    ...
    .. figure:: ../l10n_ec_edi/static/src/Code_Aduana.png
...
- In the invoice lines:
  ...
  - **Unit Value UMT**: Represents the unit price of the merchandise in the
    ...
    each of the SRI merchandise. It is transparent to the user.

  - **UMT Adduana**: This value by default is the same that is defined in the
    product of the line.


    .. figure:: ../l10n_ec_edi/static/src/invoice_line_ET.png
      :width: 700pt
...
- In the partner:
  ...
  - **Need external trade?**: Field used to indicate if the customer needs
    ...
    then the add-on is added to the SRIs for this client.


    .. figure:: ../l10n_ec_edi/static/src/partnerET2.png
  ...
  - **Colony Code**: This field is used to store the emitter's code of the
    ...
    .. figure:: ../l10n_ec_edi/static/src/partnerET.png
...
2. The cancel process in Odoo, only could be executed when the SRI status
   is cancelled.
...
When this new button is press, send the SRI to the PAC to try cancel it
...
to verify that effectively the SRI is not previously canceled in the SAT
...
1. A method will be called to verify if the SRI was properly cancelled in
the SAT system, and when the SRI status is `Cancelled` will try to cancel the
invoice in Odoo.
...
If the customer does not accept the SRI cancelation, the cancel process
must be aborted and the invoice must be returned to signed. For this, was
added an action in the invoice `Revert SRI cancellation`, that could be
...
The system executes a scheduled action that will check that the SRI status
continues `Valid` and if yes, the invoice must be returned to `Open`
(Without generate a new SRI). For this:
